...
import { supabase } from '@/lib/supabase'; // 共通のsupabaseを利用
...
export default function AuthGuard({ children }: { children: ReactNode }) {
  ...
  const verifyAuthAndPermission = async () => {
    try {
      ...
      // 2. Discord等のセッションで profiles テーブルを検索
      ...
      // ★ status が left の場合は強制的にアクセス不可（権限なし・強制ログアウト扱い）にする
      ...
      // 3. BOOLEANフラグから保持しているロールのリストを構築
